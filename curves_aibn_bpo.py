...
class PressureLimitCurves:
    def __init__(self, file_list=None):
        ...
        self.all_min_index = []
        self.all_min_value = []
        with open('settings/curves_settings_aibn_bpo.json', 'r', encoding='UTF-8') as settings_file:
            ...
            self.start_row = settings_data["start_row"]
            self.end_row = settings_data["end_row"]
            ...
            self.factor = [0.008,0.008,0.008,0.008,0.008]
            if not self.mass:
    ...
    def process_all_csv_files(self):
        for csv_file_name in tqdm(get_paths(self.file_list, self.data_folder), desc="Processing CSV Files",
            ...
            pressure_diff_points, time_points = get_max_diff(times, smoothed_pressures)
            found_index = 0
            for idx, point in enumerate(smoothed_pressures):
                if point > self.factor[self.current]:
                    found_index = idx
                    break
            self.time_start.append(times[found_index])
            # self.time_start_index.append(found_index)

            # time_of_all = np.where(times == time_points[found_index])[0]
            end_index = found_index + self.all_time * self.points_one_second

            # 提取从值为 1 的点开始向后 100 个点的数据
            pressure_subset = smoothed_pressures[found_index:end_index]
            self.y.append(pressure_subset)
        self.x = np.arange(0, 1 / self.points_one_second * len(self.y[0]), 1 / self.points_one_second)
    ...
    def get_pressure_time(self, csv_file_name):
        file_data = pd.read_csv(csv_file_name, skiprows=range(1, self.start_row * self.points_one_second),
                                nrows=self.end_row * self.points_one_second - self.start_row * self.points_one_second)
        voltages = file_data.iloc[:, 0]
        pressures = voltages / 0.725 * voltage_to_pressure_factor(self.unit)
        times = np.arange(0,
                          len(pressures)) / self.points_one_second + self.start_row * self.points_one_second / self.points_one_second
        self.points_count = len(pressures)
        return times, pressures

    def draw_main_figure(self, ax):
        for index, y in enumerate(self.y):
            plt.plot(self.x, y, label=self.mass[index], color=self.color[index])
        ...
        if len(self.y) == 1:
            plt.title(f'{self.figure_title}{self.species}({self.mass[self.current]}g)', pad=self.title_padding)
