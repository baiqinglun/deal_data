...
class BarDrawer:
    def __init__(self):
        ...
        with open('settings/bar_settings.json', 'r', encoding='UTF-8') as settings_file:
            ...
            self.y_title_padding = settings_data["y_title_padding"]
    ...
    def draw_figure(self, csv_file_name, x, mass, max_pressure, max_pressure_rise):
        ...
        sub_sequence_y = np.linspace(0, 1, 9)
        sub_sequence_y_rounded = np.round(sub_sequence_y, 2)
        sub_sequence_y2 = np.linspace(0, 40, 9)
        ...
        plt.title(f"{self.y_label[0]},{self.y_label[1]} of {self.species}", fontsize=self.font_size,pad=self.title_padding)
    ...
    def draw_first_bar(self, ax1, x, max_pressure, pressure_ax1_y_limit):
        ...
        ax1.set_ylabel(f'{self.y_label[0]}({self.pressure_unit})', color=self.color[0], fontsize=self.font_size,labelpad=self.y_title_padding)
        # 设置第一个y轴的范围
    ...
    def draw_second_bar(self, ax2, x, max_pressure_rise, pressure_ax2_y_limit):
        bars2 = ax2.bar(x + self.bar_width, max_pressure_rise, self.bar_width, label=self.y_label[1],
                        ...
                        align='center')
        # 添加标签给第二个y轴，设置字体大小
        ax2.set_ylabel(f'{self.y_label[1]}({self.pressure_unit}/{self.time_unit})', color=self.color[1],
                       fontsize=self.font_size,labelpad=self.y_title_padding)
